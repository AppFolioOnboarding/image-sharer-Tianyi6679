...
class ImagesCrudTest < FlowTestCase
  test 'add an image' do
    ...
    assert_equal 'Url is invalid', new_image_page.url_form.error_message.text

    image_url = 'https://media3.giphy.com/media/EldfH1VJdbrwY/200.gif'

    new_image_page.url_form.url.set(image_url)
    new_image_page.tags.set(tags.join(', '))
  ...
  test 'delete an image' do
    ...
    Image.create!([
      { title: 'test_img1', url: cute_puppy_url, tag_list: 'puppy, cute' },
      { title: 'test_img2', url: ugly_cat_url, tag_list: 'cat, ugly' }
    ])
  ...
  test 'view images associated with a tag' do
    ...
    Image.create!([
      { title: 'test_img1', url: puppy_url1, tag_list: 'superman, cute' },
      { title: 'test_img2', url: puppy_url2, tag_list: 'cute, puppy' },
      { title: 'test_img3', url: cat_url, tag_list: 'cat, ugly' }
    ])
